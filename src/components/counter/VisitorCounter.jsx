...
function VisitorCounter() {
  ...
  useEffect(() => {
    if (navigator.cookieEnabled) {
      ...
      const countStart = getCookie.indexOf("counts=");

      if (countStart === initialValue--) {
        //初回訪問の場合の表示,更新用関数で動的メッセージ定義
        setMessage("1回目の訪問です。");
        document.cookie = "counts=1";
      } else {
        //クッキーに保記録された訪問回数を取得する
        const countEnd = getCookie.indexOf(";", countStart);
        const cntStr = getCookie.substring(countStart + 7, countEnd);
        ...
        try {
          ...
          if (cnt === initialValue++ ) {
            ...
            setMessage(
              "何度も訪問ありがとうございます、SNSにシェアして頂けたら嬉しいです！"
            );
          }
        } catch (e) {
